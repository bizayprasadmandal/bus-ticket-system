...
export default function DispatcherLayout() {
  ...
  const getBreadcrumbs = () => {
    ...
    const crumbs = [{ label: 'Home', path: '/dispatcher' }];
    let currentPath = '';
    segments.forEach((seg, i) => {
      if (i === 0) return;
      currentPath += `/${seg}`;
      const full = `/dispatcher${currentPath}`;
              ...
              <Avatar
                ...
                fallback={
                  <span className="text-primary-600 font-semibold text-sm">
                    {user?.full_name?.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2) || 'DS'}
                  </span>
                }
              />
            </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 animate-fade-in">
        ...
        <div className="bg-white border-b">
          <div className="lg:hidden h-14 flex items-center px-4">
            ...
            <h1 className="ml-3 text-lg font-bold text-blue-600">Dispatcher Panel</h1>
          </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3 pl-4 border-l border-gray-200">
                ...
                <div className="text-right">
                  <p className="text-sm font-medium text-gray-800">{user?.full_name || 'Dispatcher'}</p>
