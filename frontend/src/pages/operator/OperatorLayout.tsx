...
const navItems = [
  ...
  { to: '/operator/schedule', label: 'Schedule', icon: Clock },
  { to: '/operator/revenue', label: 'Revenue', icon: TrendingUp },
...
export default function OperatorLayout() {
  ...
  const getBreadcrumbs = () => {
    ...
    const crumbs = [{ label: 'Home', path: '/operator' }];
    let currentPath = '';
    segments.forEach((seg, i) => {
      if (i === 0) return;
      currentPath += `/${seg}`;
      const full = `/operator${currentPath}`;
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="bg-white border-b">
          <div className="lg:hidden h-14 flex items-center px-4">
            ...
            <h1 className="ml-3 text-lg font-bold text-blue-600">Operator Panel</h1>
          </div>
